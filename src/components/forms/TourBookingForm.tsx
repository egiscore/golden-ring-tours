    ...
    <div className="bg-white rounded-2xl shadow-lg p-4 sm:p-6 border-2 border-[#D4AF37]/20">
      ...
      <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <a
            href={`https://t.me/79255693984?text=${encodeURIComponent(`Здравствуйте! Интересует тур "${tourTitle || 'Экскурсия'}". ${formData.date ? `Дата: ${formData.date}` : ''} Взрослых: ${formData.adults}, детей: ${formData.children}. Примерная стоимость: ${totalPrice.toLocaleString('ru-RU')} ₽`)}`}
            target="_blank"
            rel="noopener noreferrer"
